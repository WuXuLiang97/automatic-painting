...
class MM:
    ...
    def screenshot_OCR_str(
        self, x1, y1, x2, y2, templates_dict, sim, get_colour=None, drag=None
    ):
        """
            ...
            for digit_char, digit_templates in templates.items():
                for template_idx, template_img in enumerate(digit_templates):
                    ...
                    try:
                        ...
                        logger.info(
                            f"  匹配模板 '{digit_char}_{template_idx}' 时出错: {e}"
                        )

            # 保存有效匹配
            if best_match and best_similarity >= sim:
                logger.info(
                    f"  最佳匹配: '{best_match}' (相似度 = {best_similarity:.4f})"
                )
        ...
        if time_s is None:
            ...
            while (
                ...
                if elapsed_time >= time_s:
                    ...
                    return []
                logger.info(
                    f"在 {time_s - elapsed_time} 秒后超时，未找到图片:{img_name}。"
                )
    ...
    def FindPic(
        ...
        for id, img_name in enumerate(img_name_array):
            ...
            for xy in xy_array:
                ...
                self.print_and_append_coordinates(
                    img_name, values[count], xy, w, h, pard, x1, y1, id
                )

                count += 1

                if drag == 1:
                    self.screenshot_show_image = cv2.cvtColor(
                        self.screenshot_show_image, cv2.COLOR_BGRA2BGR
                    )
                    # 如果drag为True,则在目标图像上绘制矩形框标记模板位置
                    self.draw_rectangle_if_needed(
                        self.screenshot_show_image, xy, w, h, drag
                    )
                elif drag == 2:
    ...
    def draw_rectangle_if_needed(
        self, Color_Image, maxLoc, template_width, template_height, drag
    ):
        """
        如果drag为True，则在目标图像上绘制矩形框标记模板位置。
        ...
        if drag:
            ...
            # 打印将要绘制的矩形的坐标和颜色
            logger.info(
                f"{(x, y), (x + template_width, y + template_height), (0, 255, 0)}"
            )
            # 在Color_Image上绘制矩形
            cv2.rectangle(
                Color_Image,
