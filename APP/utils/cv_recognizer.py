...
class MM:
    ...
    def FindPic(self, x1: int, y1: int, x2: int, y2: int, img_name, sim=0.7, mc=False, drag=None,
                delta_color=([0, 0, 0], [179, 255, 255]), add_offset=True, img_numpy=None):
        """
        优化后的图像识别方法，增加了亮度适应性处理
        """
        screenshot_np = None
        start_time = timeit.default_timer()  # 获取当前时间作为开始时间
        # 初始化一个列表来存放返回结果
        ...
        # 尝试多种亮度/对比度组合
        brightness_levels = [-50, 0, 50]
        contrast_levels = [-50, 0, 50]

        img_name_array = img_name.split("|")
        found = False  # 新增标志位，用于控制循环中断
        ...
        for id, img_name in enumerate(img_name_array):
            ...
            for brightness in brightness_levels:
                if found and not mc:  # 如果已经找到且不需要多个结果，则中断
                    break
                for contrast in contrast_levels:
                    if found and not mc:  # 同上
                        break
                    adjusted_img = adjust_brightness_contrast(screenshot_np.copy(), brightness, contrast)
                    # print("亮度处理完毕")
                    # 颜色过滤处理
                    ...
                    upper_color = np.array(delta_color[1])
                    mask = cv2.inRange(hsv_image, lower_color, upper_color)
                    color_filtered_image = cv2.bitwise_and(adjusted_img, adjusted_img, mask=mask)
                    gray_filtered_image = cv2.cvtColor(color_filtered_image, cv2.COLOR_BGR2GRAY)
                    ...
                    # 使用归一化相关系数匹配方法
                    result = cv2.matchTemplate(gray_filtered_image, template_image, cv2.TM_CCOEFF_NORMED)
                    loc = np.where(result >= sim)
                    values = result[loc[0], loc[1]]
                    ...
                    if len(xy_array) > 0:
                        ...
                        # 只取第一个匹配（如果 mc=False）
                        if not mc:
                            xy_array = [xy_array[0]]  # 只保留第一个匹配
                            values = [values[0]]  # 只保留第一个相似度值
                        for xy in xy_array:
                            pard = random.randint(0, 2)
                            self.print_and_append_coordinates(img_name, values[count], xy, w, h, pard, x1, y1, id)
                            count += 1
                            ...
                            if drag == 1:
                                self.screenshot_show_image = cv2.cvtColor(self.screenshot_show_image,
                                                                          cv2.COLOR_BGRA2BGR)
                                self.draw_rectangle_if_needed(self.screenshot_show_image, xy, w, h, drag)
                            elif drag == 2:
                                self.screenshot_show_image = color_filtered_image
