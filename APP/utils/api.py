...
def test_update_subgroup_config(cookies, subgroup_name, old_order, new_data):
    ...
    for attempt in range(max_retries):
        try:
            response = requests.put(url, json=new_data, cookies=cookies, timeout=30)
            ...
            if attempt < max_retries - 1:
                ...
                logging.error(f"更新分组配置失败: {e}")
                # 返回错误信息而不是抛出异常
                return {"error": True, "message": f"连接失败: {str(e)}"}

        except Exception as e:
            logging.error(f"更新分组配置时发生错误: {e}")
...
if __name__ == "__main__":
    # run_tests()
    cookies = test_login('wuzl666', 'wuzl666')
