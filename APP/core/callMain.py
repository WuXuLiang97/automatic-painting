...
class AppMain(QMainWindow, Ui_MainWindow):
    ...
    def init_content(self):
        ...
        # 设置角色表格的列数为7
        self.rolesTable.setColumnCount(9)

        # 设置角色表格的水平头部标签
        header_labels = ['位置', '职业类型', '转职职业', '身高','进图移速', '地图名称', '难度', '刷完？', '预留疲劳']
    ...
    def update_roles_table_data(self):
        ...
        try:
            ...
            print(f"update_roles_table_data:{ret}")
            # 检查是否有配置数据
            ...
            for index, (role_id, role_data) in enumerate(sorted_roles):
                ...
                self.rolesTable.setItem(index, 4, QTableWidgetItem(str(role_data.get('moving_speed', ''))))
                self.rolesTable.setItem(index, 5, QTableWidgetItem(role_data.get('map', '')))
                self.rolesTable.setItem(index, 6, QTableWidgetItem(str(role_data.get('difficulty', ''))))
                self.rolesTable.setItem(index, 8, QTableWidgetItem(str(role_data.get('leave_pl', ''))))
                ...
                if expire_time:
                    try:
                        ...
                        if today != expire_date:
                            self.rolesTable.setItem(index, 7, QTableWidgetItem("否"))
                        else:
                            self.rolesTable.setItem(index, 7, QTableWidgetItem("是"))
                    except ValueError:
                        # 日期格式错误
                        self.rolesTable.setItem(index, 7, QTableWidgetItem("未知"))
                else:
                    self.rolesTable.setItem(index, 7, QTableWidgetItem("否"))
        except Exception as e:
            print("更新角色表格数据异常:", e)
