...
class PlayerThread(QThread):
            ...
            while self.running:
                ...
                if pl_value is not None and isinstance(pl_value, (int, float)) and pl_value <= self.player.pl_value:
                    ...
                    dic_data = {'career': role_settings['career'],
                                ...
                                'moving_speed': self.player.moving_speed,
                    ...
                    continue
                self.mm.FindPic_sleep(861, 544, 944, 582, "商城图标.bmp", 0.9, time_s=10, my_sleep=0.5)
                ...
                while self.brush_running:
                    if self.player.map_name not in ("深渊：终末崇拜者", "深渊：最终调律者"):
                        ...
                        if any(self.similarity(cleaned_text, boss) >= 0.7 for boss in target_boss):
                            break
                        else:
    ...
    def read_current_role_config(self):
        ...
        if moving_speed is not None and moving_speed != 0:
            self.player.moving_speed = int(moving_speed)
        else:
            self.player.moving_speed = None
        # 从角色设置中读取地图名称，并更新到玩家对象中
        self.player.map_name = role_settings["map"]
        if self.player.map_name in ("深渊：终末崇拜者", "深渊：最终调律者"):
            self.send_log(f"深渊")
            gv.sy = True
        else:
            gv.sy = False
        ...
        if self.is_boss:  # 判断是否处于Boss房间
            ...
            while self.brush_running and not self.ghost_state:  # 循环条件：刷子正在运行且非幽灵状态
                ...
                if execution_time > 120:  # 如果执行时间超过60秒
                    ...
                    break  # 退出循环
                if len(self.monsters) > 0 or self.player.map_name in ("深渊：终末崇拜者", "深渊：最终调律者") and self.is_boss and not self.has_continue:  # 如果怪物列表不为空
                    self.attach_monster()  # 攻击怪物的方法
                else:  # 如果怪物列表为空
                    if self.player.map_name in ("深渊：终末崇拜者", "深渊：最终调律者") or not self.doors:
    ...
    def process_detect_message(self, cls, game_image):
        ...
        for data in cls:
            ...
            # 处理怪物和Boss
                ...
                if data[0].startswith("boss_sy") and self.player.map_name in ("深渊：终末崇拜者", "深渊：最终调律者") and self.to_door_count >= 3:
            ...
            elif data[0].startswith("forward") and self.player.map_name in ("深渊：终末崇拜者", "深渊：最终调律者"):
                self.forward = True
                if data[1] > 1067 / 2:
    ...
    def deposit_goods(self):
        if self.player.map_name in ("跌宕群岛", "妖气追踪","生命巡礼","怀纳千海之天",):
            return
    ...
    def process_pass(self):
        ...
        if self.player.map_name not in ("深渊：终末崇拜者", "深渊：最终调律者"):
            ...
            if ret:
                ...
                if self.brush_cnt % 16 == 0 and self.player.map_name not in ("深渊：终末崇拜者", "跌宕群岛", "妖气追踪","生命巡礼","怀纳千海之天", "深渊：最终调律者"):
                    self.operator_module.sale_goods(self.sell)
                pl_value = self.operator_module.ocr_pl(self.get_text, self.send_log)
                # 识别到疲劳且小于预留
                if pl_value is not None and isinstance(pl_value, (int, float)) and pl_value <= self.player.pl_value:

                    if self.brush_cnt % 16 != 0 and self.player.map_name not in ("深渊：终末崇拜者", "跌宕群岛", "妖气追踪","生命巡礼","怀纳千海之天", "深渊：最终调律者"):  # 这几个图不出售
                        # 出售装备、材料
                        self.operator_module.sale_goods(self.sell)
                    # update_role_brush_date(self.current_role_group, self.current_role_index)
                    role_settings = self.all_role_settings[self.current_role_index]
            ...
            if self.has_continue:
                ...
                ret = self.mm.is_colored(min_img, 15)#版本巨坑，更新变为28 29左右
            ...
            if self.player.map_name == "风暴幽城":
                ...
                while self.brush_running:
                    ...
                    if ret:
                        ...
                        time.sleep(0.2)
                        continue
            elif self.player.map_name == "深渊：最终调律者":
                stat_time = time.time()
                while self.brush_running:
                    ret = self.mm.FindPic(44, 84, 221, 575, "深渊调律之边界.bmp", 0.9)
                    if ret:
                        x, y = ret[0][1], ret[0][2]
                        self.operator_module.move_to(x, y)
                        pyauto.click()
                ...
                while self.brush_running:
                    ...
                    如果没到拉比谢尔则再次打开传送阵，进行传送
                    """
                    ret = self.waiting_for_the_text_to_appear([934, 23, 1013, 56], "最后的圣地", r'[\u4e00-\u9fa5]+', 15)
                    if not ret:
                        click_status = self.operator_module.click_menu_item("传送阵")
                        if not click_status:
                            logger.info("点击传送阵失败")
                        ...
                        while self.brush_running:
                            ...
                            if time.time() - stat_time > 10:
                                break
                    elif self.similarity(ret, "最后的圣地") >= 0.7:
                        break
                    time.sleep(5)
                pyauto.keyDownChar("right")
                time.sleep(random.uniform(0.8, 1.1))
                    ...
                    if ret:
                        ...
                        while self.brush_running:
                            ...
                            if time.time() - syst > 20:
                                ...
                                time.sleep(0.1)
                                return 0
                            if self.similarity(cleaned_text, "深渊：最终调律者") >= 0.7:
                                break
                            else:
                                self.send_log("深渊图检测2——未检测到在图中,等待...")
                                pyauto.keyPressChar("space")
                        ...
                        time.sleep(0.2)
                        continue
            elif self.player.map_name == "生命巡礼":
                stat_time = time.time()
                while self.brush_running:
                    ret = self.mm.FindPic(44, 84, 221, 575, "深渊调律之边界.bmp", 0.9)
                    if ret:
                ...
                if ret:
                    ...
                    time.sleep(0.1)
                while self.brush_running:
                    ret = self.mm.FindPic(64, 281, 248, 326, "生命巡礼.bmp", 0.9)
                    if ret:                        
                        # 初始化地图
                        self.room_info_map = deepcopy(a_mapInfo.get(self.player.map_name))
                        logger.info('初始化地图')
                        ...
                        time.sleep(0.2)
                        continue
            elif self.player.map_name == "怀纳千海之天":
                stat_time = time.time()
                while self.brush_running:
                    ret = self.mm.FindPic(44, 84, 221, 575, "深渊调律之边界.bmp", 0.9)
                    if ret:
                ...
                if ret:
                    ...
                    time.sleep(0.1)
                while self.brush_running:
                    ret = self.mm.FindPic(30,283,308,326, "怀纳千海之天.bmp", 0.9)
                    if ret:
                        time.sleep(0.05)
                        pyauto.keyDownChar("shift")
