...
class OperatorModule:
    ...
    def is_start_game_interface(self):
        text = self.player_instance.get_text(489,553,578,577)
    ...
    def is_celia_room(self, fun):
        ret = fun([891,550,930,575], '商城', r'[\u4e00-\u9fa5]+', 2)
        if ret:
            return True
        else:
            return False
    ...
    def find_pic_sleep(self, pic_name, x1, y1, x2, y2):
        return self.mm.FindPic_sleep(x1, y1, x2, y2, pic_name, 0.9, drag=None, delta_color=([20, 71, 204], [24, 91, 224]), time_s=2)
    ...
    def click_menu_item(self, item_name):
        ...
        while True:
            ...
            if item_name == "选择角色":
                ...
                time.sleep(0.1)
                if not self.find_pic_sleep("选择角色.bmp", 326,481,697,540):
                    pyauto.keyPressChar("esc")
                    time.sleep(0.2)
                    continue
            if item_name == "返回城镇":
    ...
    def ocr_pl(self, func: callable, func1: callable):
        ...
        for i in range(5):
            try:
                self.move_to(849,581)
                time.sleep(0.1)
                # 用传进来的方法识别
                results = func(837,550,929,568, amplify=True)
