        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("""
                INSERT INTO subgroup_config (
                    ...
                    height,moving_speed, map, difficulty, today_task_completed, everyday_tasks,
...
def add_subgroup_config(subgroup_name):
    ...
    data = request.get_json()
    required_fields = ['brush_order', 'career', 'convert_career', 'height','moving_speed', 'map', 'difficulty']
    ...
    try:
        with conn.cursor() as cursor:
            ...
            """
            values = (session['user']['username'], subgroup_name, data['brush_order'], data['career'], data['convert_career'], data['height'], data['moving_speed'], data['map'], data['difficulty'], data.get('today_task_completed', '是'), data.get('everyday_tasks', '是'), data.get('leave_pl', 0), data.get('brush_map_expire_time', '2025-01-01 23:59:59'))
...
def update_subgroup_config(subgroup_name, brush_order):
    ...
    # 检查必填字段
    required_fields = ['career', 'convert_career', 'height','moving_speed', 'map', 'difficulty']
    if not all(field in data for field in required_fields):
        return jsonify({"error": "缺少必要参数"}), 400
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(update_query, (data['career'], data['convert_career'], data['height'], data['moving_speed'], data['map'], data['difficulty'], data.get('today_task_completed', '是'), data.get('everyday_tasks', '是'), data.get('leave_pl', 0), expire_time or '2025-01-01 23:59:59', new_brush_order,  # 新的刷图序号
