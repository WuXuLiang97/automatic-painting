...
MODEL_PATH = r"D:\automatic-painting0501\PadYolo\yolo\model_data\min_map_best.onnx"
# 测试图片路径
IMAGE_PATH = r"D:\automatic-painting0501\PadYolo\1.bmp"
# TensorRT引擎保存路径（首次运行自动生成）
ENGINE_PATH = r"D:\automatic-painting0501\PadYolo\yolo\model_data\min_map_best.engine"

# 模型类别名称列表（必须与训练时完全一致）
MIN_MAP_CLASS_NAMES = ['map_hero', 'map_boss', 'map_query', 'map_elite', 'map_special', 'map_query_1']  # 示例，请根据实际模型修改
# 模型输入尺寸（必须与导出ONNX时一致）
INPUT_SHAPE = (640, 640)
# 置信度阈值（低于该值的检测框直接过滤）
CONF_THRESH = 0.25
...
def postprocess(output, img, ratio, top, left):
    ...
    for i in indices:
        ...
        conf = scores[i]
        # 获取类别名称
        name = MIN_MAP_CLASS_NAMES[cls_id] if 0 <= cls_id < len(MIN_MAP_CLASS_NAMES) else f"Unknown{cls_id}"
        # 绘制矩形框
